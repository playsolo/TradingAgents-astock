...
def create_hot_money_tracker(llm):
    ...
    def hot_money_tracker_node(state):
        ...
        system_message = (
            ...
            "\n\n📌 数据口径（必须遵守，防止量价误读）："
            "\n- get_stock_data 返回的 K 线含 `ChangePct` 列 = 官方日涨跌幅（今收 vs 上一交易日收）。描述某日「涨跌幅/单日下跌 x%」必须引用该列，禁止用 `(收盘−开盘)/开盘` 代替或自行心算。"
            "\n- K 线形态只由当日自身 O/H/L/C 判定：上影=High−max(Open,Close)，下影=min(Open,Close)−Low。禁止把前一日高/低点写成本日影线；High 与 Open 接近时不是「长上影」。"
            "\n- Volume 单位是股（如 54,223,300 股 = 5,422 万股 ≈ 0.54 亿股）；成交额单位为元、须另算（金额≈成交量×均价），禁止把成交量(股)当作成交额(元)引用。"
            "\n\n📋 必采清单 — 以下数据点必须出现在报告中；"
            "行业全市场排名失败但有个股板块兜底、或资金流仅有分时/异源日度，"
            "请按可得信息表述，**禁止**因 SoftGap 写 `[数据缺失: …]`。"
            "仅当核心工具调用失败、超时或返回 Error 时才标注 [数据缺失: xxx]："
